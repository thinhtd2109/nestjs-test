...
export class AuthService {
    ...
    async signIn(username: string, password: string) {
        ...
        if (_.trim(user.passwordHash) !== _.trim(password)) {
            await this.userRepository.updateUserCountSignIn(quantity_fail, user.id);
            return {
                status: false,
        ...
        await this.userRepository.updateUserCountSignIn(0, user.id);
        return {
            ...result.dataValues,
    ...
    async signUp(data: SignUpDto) {
        ...
        try {
            const validated = validateSignUp(data);
            if (!validated.status) {
                return validated
            }
